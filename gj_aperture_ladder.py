...
SECOSW_INIT = -0.0959
OUT_NPZ = "gj_ladder_ap%d_e%s.npz" % (AP, sys.argv[2])
...
class Model:
    def __init__(self, visits):
        ...
        self.off = np.cumsum([3] + self.npv[:-1])
        self.ndim = 3 + sum(self.npv)
...
def main():
    ...
    m = Model(visits)
    fp0 = 140e-6
    x0 = np.array([fp0, SECOSW_INIT, 0.0] + m.seed_sys(fp0, SECOSW_INIT))
    nw = 3 * m.ndim
    ...
    p0[:, 2] = 1e-6 * rng.standard_normal(nw)
